...
Optional statuses to filter for. Defaults to `undefined`.
...
`CampaignOrderBy.Apr` is supported only for `rewards` campaigns. Defaults to
`undefined`.
...
An optional field that specifies the sort direction of the results; Defaults to
`undefined`.
