...
Reward Ratio, an optional step in the Metrom campaign builder, lets you
...
- **Fees** – rewards positions that are earning (i.e., in‑range, concentrated
  ...
  > **Note**: If reward ratio is not configured, Metrom will default to 100% for Fees.
...
1. **In Campaign creation form**, expand the **Define the rewards to distribute** section.
2. **Enter percentages for the respective tokens**, and click **Next** to save the configuration.
...
3. That’s it—no further action needed. You can now **enable KPI** or
   **Range-based incentives**, or skip ahead and see the **Campaign Preview** and connect wallet to 
   approve the reward tokens and launch the campaign.
