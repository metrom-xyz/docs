...
Once all the required form steps are completed, it's time to **SET
RANGE TO REWARD** and explore the available configuration options.

Once expanded, the interface shows a range visualizer with the current price
...
In the **USDC-USDT pair**, the **current price is 1 USDC = 1.002 USDT**. If you
click on USDT, it will show the current price of 1 USDT = 0.9995 USDC.

**Min Price we set is 0.99** and we set the **Max price** to 1.01.
...
Once you’re satisfied with the **Range-based incentivization** setup, save the
changes and check the **campaign preview** on the right side, where you can review all the details.

<div className="mediaWrapper">
  ![Preview with RBI](/images/rbi-example-campaign.png)
</div>

At this point, the only thing left to do is **approve the reward tokens** and **launch the campaign**.
